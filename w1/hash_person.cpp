...
struct Address {
  ...
  bool operator==(const Address& other) const {
      return ((tie(this->city, this->street) == tie(other.city, other.street)) && this->building == other.building);
  }
};
...
struct AddressHasher {
    ...
    size_t a_1 = shash(address.city);
    size_t a_2 = shash(address.street);
    size_t a_3 = ihash(address.building);
...
size_t operator() (const Person& person) const {
    ...
    size_t b = ihash(person.height) + dhash(person.weight);
    size_t c = ahash(person.address);
    size_t x = 2'123'456;
